...
// 四化改用「中州派」對應表（使用者提供之教材表格，十天干逐一驗證）。與 iztro 預設表僅庚干有別
// （中州庚科＝天府，iztro 預設＝太陰），其餘九干相同。注意：不啟用 iztro 的 algorithm:'zhongzhou'
// ——實測該選項會把命主表也換掉（1976 盤命主 破軍→廉貞），與星僑參考盤（命主破軍）矛盾，
// 故只換四化表、其餘沿用預設演算法。
iztro.astro.config({ mutagens: {
  甲: ["廉貞", "破軍", "武曲", "太陽"], 乙: ["天機", "天梁", "紫微", "太陰"],
  丙: ["天同", "天機", "文昌", "廉貞"], 丁: ["太陰", "天同", "天機", "巨門"],
  戊: ["貪狼", "太陰", "右弼", "天機"], 己: ["武曲", "貪狼", "天梁", "文曲"],
  庚: ["太陽", "武曲", "天府", "天同"], 辛: ["巨門", "太陽", "文曲", "文昌"],
...
// 2026-07-18 改為官網 /qimen 前端原始碼的原始表（szwjtin／szwjdoor／szwjgod，該頁純前端、免登入）：
//   神假 天盤干應為 丁／己／癸（先前只寫「己」→ 漏判丁、癸）
//   鬼假 天盤干應為 丁／己／癸（先前寫「天盤或地盤含癸」→ 既漏判丁己、又誤判地盤癸）
const WU_JIA_RULES = [
  { name: "天假", men: "景", tianGans: ["乙", "丙", "丁"], shens: ["九天"] },
  { name: "地假", men: "杜", tianGans: ["丁", "己", "癸"], shens: YIN_SHEN },
  { name: "人假", men: "驚", tianGans: ["壬"], shens: ["九天"] },
  { name: "神假", men: "傷", tianGans: ["丁", "己", "癸"], shens: ["九地"] },
  { name: "鬼假", men: "死", tianGans: ["丁", "己", "癸"], shens: ["九地"] }
];
...
// 2026-07-18 全表改為官網 /qimen 前端原始碼的原始定義（變數 qmjd／qmjdgod／qmjdtin／
// qmjddoor／qmjddei／qmjdgong；該頁純前端計算、免登入，直接抄錄不再反推）。官網判斷式：
//   神(空=不限) && 天盤干相等 && 門在清單內 && 地盤干(空=不限) && 宮位(空=不限)
// 修正兩條：
//   天遁 官網＝天盤丙＋生門＋「地盤丁」、無神條件（先前寫成 丙+生門+神(九天/六合) 兩處都錯：
//        少了地盤丁、多了神條件。舊註解「掃10種神只有九天/六合觸發」是巧合——那些樣本地盤剛好是丁）
//   虎遁 官網＝天盤乙＋「休門」＋地盤辛＋艮8宮（四條件全中，極罕見）。先前猜「乙＋三吉門＋艮8」
//        被否證後停用，其實只是門與地盤干條件抓錯：08-19 艮8 乙/辛 是「開門」、09-09 艮8 乙/休
//        是「地盤壬」，兩筆都各差一個條件，難怪掃不到正例。現依原始碼補回。
const JIU_DUN_RULES = [
  { name: "天遁", tianGan: "丙", men: "生", diGan: "丁" },
  { name: "地遁", tianGan: "乙", men: "開", diGan: "己" },
  { name: "人遁", tianGan: "丁", men: "休", shen: "太陰" },
  { name: "神遁", tianGan: "丙", men: "生", shen: "九天" },
  { name: "鬼遁", tianGan: "乙", men: "杜", shen: "九地" },
  { name: "風遁", tianGan: "乙", mens: ["開", "休", "生"], gong: 4 },
  { name: "雲遁", tianGan: "乙", mens: ["開", "休", "生"], diGan: "辛" },
  { name: "龍遁", tianGan: "乙", mens: ["開", "休", "生"], gong: 1 },
  { name: "虎遁", tianGan: "乙", men: "休", diGan: "辛", gong: 8 }
];
