...
// H. 天盤干入墓：改用使用者給的兩條規律——
//    規律一：四維宮（隅宮）各藏兩個地支：巽四宮(東南)藏辰、巳；坤二宮(西南)藏未、申；
//            乾六宮(西北)藏戌、亥；艮八宮(東北)藏丑、寅。四正宮（坎一、震三、離九、兌七）不藏辰戌丑未，
//            不會有天盤干入墓。
//    規律二：五行大墓固定在辰戌丑未，只有這 5 個天干有墓：火土墓戌（丙、戊見乾宮入墓）、
//            水墓辰（壬見巽宮入墓）、金墓丑（庚見艮宮入墓）、木墓未（乙見坤宮入墓）。
//    這個表跟原本「甲乙丙丁戊己庚辛壬癸每個都找得到墓」的十二長生陰陽同墓版本不一樣，只有 5 個天干
//    真的有墓；乙不對乾、癸不對坤，剛好順便解開先前「命宮乾六宮天干乙、命宮坤二宮天干癸都沒出現
//    入墓字樣」這兩筆資料的矛盾，不需要再額外用「命宮不算入墓」這種例外規則
const RUMU_STEMS = { 乾: ["丙", "戊"], 艮: ["庚"], 巽: ["壬"], 坤: ["乙"] };
...
function buildCornerWords(g, men, xing, shen, tianGan, isXiongDi, isZiNu, isYiMaGong) {
  const words = [];
  ...
  // 入墓改用新的 RUMU_STEMS（只有丙戊壬庚乙 5 個天干真的有墓），不需要再排除命宮，見上方 H 段註解
  if ((RUMU_STEMS[gua] || []).includes(tianGan)) words.push({ text: tianGan + "入墓", type: "rumu" });
  return words;
}
...
function calculateQimenHeader({ year, month, day, hour, minute, name, gender }) {
  ...
  [1, 2, 3, 4, 6, 7, 8, 9].forEach((g) => {
    ...
    gongs[g] = {
      ...
      cornerWords: buildCornerWords(g, renPanMen[g], tianPanXing[g], shenPan[g], tianGan, g === xiongdiGong, g === ziNuGong, g === yiMaGong)
    };
  });
