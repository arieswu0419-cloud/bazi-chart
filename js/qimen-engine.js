...
// ---- 表頭「格局」四條規則：以下全部改為官網 /qimen 前端原始碼的原始對照表（2026-07-18）----
// 這幾張表是直接讀官網頁面內嵌 JS（該頁純前端計算、免登入）抄錄的，不再是反推猜測。
// 官網變數名對照：tianfu／yunv／sgrm／wbys／twsz。

// 天輔時：tianfu[日干] === 時支（比對「地支」，與時干無關）。
// 先前兩版都錯：舊版用「日干→甲時(干支)」、v196 用「天輔星落時支宮」，都是被巧合誤導
// （多數樣本剛好是甲時）。官網原始碼證實只看日干配時支。
const TIANFU_SHI_MAP = { 甲: "巳", 乙: "申", 丙: "午", 丁: "辰", 戊: "寅", 己: "巳", 庚: "申", 辛: "午", 壬: "辰", 癸: "寅" };
// 玉女時：yunv[時干] === 時支，即時柱為 庚午／戊子／丙午／己卯／丁酉／乙卯 六柱之一。
// 先前用「值使門落地盤丁奇宮」的盤面規則（isYuNuShouMen），與官網無關，已停用。
const YUNV_SHI_MAP = { 庚: "午", 戊: "子", 丙: "午", 己: "卯", 丁: "酉", 乙: "卯" };
// 五不遇時：wbys[日干] === 時柱干支。官網 庚日只有「丙子」一個，先前多列的「丙戌」會誤判，已移除。
const WUBUYU_SHI_MAP = { 甲: "庚午", 乙: "辛巳", 丙: "壬辰", 丁: "癸卯", 戊: "甲寅", 己: "乙丑", 庚: "丙子", 辛: "丁酉", 壬: "戊申", 癸: "己未" };
// 天網四張：官網 twsz 把 酉未巳卯丑亥 六支全對到「癸」，判斷式為 twsz[時支] === 時干，
// 等價於「時干為癸」（癸只會配這六支）。先前用日干→癸時對照表，會漏判戊日癸亥時等，已改。

// 時幹入墓：sgrm[時干] === 時支，即時柱為 戊戌／壬辰／丙戌／癸未／丁丑／己丑 六柱之一。
// 這正是先前反推出的「時柱自坐墓庫」規則——官網原始碼只列六柱，因為甲未／乙戌／庚丑／辛辰
// 這四組在六十甲子裡根本不存在，兩者完全等價（也解釋了最早「時干乙誤標」的懸案）。
const SHIGAN_MU_ZHI = { 戊: "戌", 壬: "辰", 丙: "戌", 癸: "未", 丁: "丑", 己: "丑" };
...
function detectQimenPatterns({ gongs, xingDelta, menDelta, dayGan, timeGan, timeZhi, diPan, xingTargetGong, menTargetGong }) {
  ...
  // 以下四條全部照官網原始碼的對照表（見上方註解區）
  if (TIANFU_SHI_MAP[dayGan] === timeZhi) labels.push("天輔時");
  if (YUNV_SHI_MAP[timeGan] === timeZhi) labels.push("玉女時");
  if (SHIGAN_MU_ZHI[timeGan] === timeZhi) labels.push("時干入墓");
  if (WUBUYU_SHI_MAP[dayGan] === timeGanZhi) labels.push("五不遇時");
  if (timeGan === "癸") labels.push("天網四張");
