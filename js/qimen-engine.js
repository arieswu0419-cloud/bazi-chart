...
// H. 天干入墓：十天干除「甲」隱遁外，其餘 9 個天干各自的五行墓庫固定在辰戌丑未，落在對應的
//    四維宮（隅宮）就入墓：丙火、戊土墓戌→乾六宮(西北)；丁火、己土、庚金墓丑→艮八宮(東北)；
//    辛金、壬水墓辰→巽四宮(東南)；癸水、乙木墓未→坤二宮(西南)。四正宮（坎一、震三、離九、兌七）
//    不藏辰戌丑未，不會有入墓。不論天盤干或地盤干，只要落入對應宮位就各自觸發（兩個都觸發就各自
//    顯示一次，同干只顯示一次）
const RUMU_STEMS = { 乾: ["丙", "戊"], 艮: ["丁", "己", "庚"], 巽: ["辛", "壬"], 坤: ["癸", "乙"] };
...
function buildCornerWords(g, men, xing, shen, tianGan, diGan, isXiongDi, isZiNu, isYiMaGong) {
  ...
  // 入墓：天盤干、地盤干各自判斷，同一個字重複（天地盤同干）只顯示一次
  const rumuList = RUMU_STEMS[gua] || [];
  if (rumuList.includes(tianGan)) words.push({ text: tianGan + "入墓", type: "rumu" });
  if (diGan !== tianGan && rumuList.includes(diGan)) words.push({ text: diGan + "入墓", type: "rumu" });
  return words;
}
