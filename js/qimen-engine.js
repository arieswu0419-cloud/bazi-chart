...
// 「天輔時」＝日干對應的「甲時」（五鼠遁下該日時干為甲的時辰），但甲／己日不觸發。
// 定案過程（2026-07-18，官網 qimen-xuanpan 命盤模式實測，全部用「命盤分析」鈕重新送出取得）：
//   ・丙子日甲午時（2026-07-01 11:17）→ 官網顯示天輔時 ✓ 符合本表 丙→甲午
//   ・丁巳日甲辰時（2026-08-11 08:17，使用者截圖）→ 顯示 ✓ 符合 丁→甲辰
//   ・甲午日甲戌時（07-19 19:17）／己卯日甲戌時（07-04 19:17，使用者截圖）→ 皆不顯示，
//     故移除舊表的 甲→甲戌、己→甲戌 兩條（甲己日五鼠遁起甲子，一天有甲子與甲戌兩個甲時，
//     官網兩者都不標，直接不列入本表即可）
// 註：曾短暫改成「天輔星落時支宮」（v196），已被 07-01 11:17 這筆推翻（該盤天輔在巽4、
//   時支午對離9，星落宮規則會漏標）——已還原為本表。乙／辛／壬／戊／癸 各條沿用原表尚未逐一實測。
const TIANFU_SHI_MAP = { 乙: "甲申", 庚: "甲申", 丙: "甲午", 辛: "甲午", 丁: "甲辰", 壬: "甲辰", 戊: "甲寅", 癸: "甲寅" };
...
function detectQimenPatterns({ gongs, xingDelta, menDelta, dayGan, timeGan, timeZhi, diPan, xingTargetGong, menTargetGong }) {
  ...
  // 天輔時：日干對應的甲時（見上方 TIANFU_SHI_MAP 註解；甲／己日不在表中＝不觸發）
  if (TIANFU_SHI_MAP[dayGan] === timeGan + timeZhi) labels.push("天輔時");
