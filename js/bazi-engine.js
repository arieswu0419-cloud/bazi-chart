...
// 丑年支原值錯誤，已用實測結果改正（丑：酉→巳）。戌年支先前誤改成「卯」，這輪拿莊振豪／何姵璇
// （兩人年柱都是壬戌）的大運流年資料重新核對：庚子／壬子（子支）都命中血刃，癸卯／乙卯（卯支）都沒有命中，
// 證實應該是「戌：子」，不是「卯」，已修正回來。其餘尚未逐一測完，如仍有出入見 README
const XUE_REN = { 子: "戌", 丑: "巳", 寅: "申", 卯: "未", 辰: "午", 巳: "巳", 午: "辰", 未: "卯", 申: "寅", 酉: "丑", 戌: "子", 亥: "亥" };
// 血刃還有另一組以「月支」為準的查表（跟上面以年支查的 XUE_REN 是兩條獨立規則，命中任一條都算），
// 大運流年的血刃常常是這條規則命中、年支那條沒抓到，目前測到 6 個月支，其餘（午／申／子／丑）還沒確認。
// 巳月原本誤測成「巳」（月支自我對應），拿何姵璇 1982-05-30（月支巳）這筆資料的四柱測試，證實不是巳；
// 後來比對她的大運流年表才找到正確值：大運「丁酉」、流年「己酉」都命中血刃，其餘所有柱（含四柱本身）都沒有，
// 改成「巳：酉」，已修正上線。未月原本誤測成「辰」，拿莊振豪 1982-07-26（月支未）這筆資料的大運核對，
// 大運「丙辰」參考網站沒有血刃（只有福星／國印／流霞／大耗），證實「未：辰」是錯的，已移除，改標未確認。
const XUE_REN_MONTH = { 寅: "丑", 卯: "未", 辰: "寅", 巳: "酉", 戌: "申", 亥: "丑" };
...
function getShenShaForPillar(pillarRole, targetGan, targetZhi, ctx) {
  ...
  if (targetZhi) {
    ...
    // 血刃另一條規則：以月支查、對照其他柱，日柱本身不算目標（實測發現跟日支剛好同支時不會命中）
    if (pillarRole !== "day" && XUE_REN_MONTH[monthZhi] === targetZhi) push("血刃");
